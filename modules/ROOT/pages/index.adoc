...
*Use `id` if and only if the resource is a real-world concept or entity*.
--
****
...
In a `GET` request to the `id` IRI one can make use of _content negotiation_ to clarify what type of representation is desired. The web server can then use a _303 See Other_ redirect to the IRI of an information resource which provides the appropriate representation.
...
NOTE: Of course, if one knows the IRI of some desired representation, this IRI can be used directly instead of using the redirection mechanism.
