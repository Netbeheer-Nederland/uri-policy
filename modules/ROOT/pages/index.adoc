...
`\{kind}`::
Resource kind: `data` | `id` | `www` +
`\{category}`::
<<_data_products,`data-product`>> | <<_documentation,`documentation`>> | <<_models,`ontology`>> | <<_models,`register`>> | <<_models,`schema`>> | <<_models,`thesaurus`>>
`\{namespace}`::
