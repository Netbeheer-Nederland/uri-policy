...
:fn-doc-iri: pass:c,q[footnote:doc-iri[For those who wonder if it is really necessary to specify both the `www` URI type and the `documentation` category: yes it is. The `www` URI type indicates we are requesting a web page representation of the resource. The `documentation` category tells us humans that the resource is a document. Contrast this, for example, with a web page representation of an ontology term, which would have a `www` URI type but an `ontology` container type.]]
...
== Background
A well-defined URI strategy is crucial to ensuring that resources on the web are uniquely and consistently identified, enabling the reliable linking, discovery, and integration of data. As the industry generates and shares large volumes of data across various stakeholders, a standardized URI strategy is essential to achieving data interoperability, scalability, and long-term sustainability.
...
This document is normative for -- and only for -- creating URIs for resources defined or maintained by Netbeheer Nederland (NBNL).
...
[#resources]
== Resources
A <<def-resource>> can be anything we wish to state something about on the web: a certain web page, a company or the color "red". To make statements about a resource, one can refer to it by its URI.
...
The URI that identifies a resource will sometimes be referred to as its _identity URI_, to distinguish it explicitly from URIs that identify representations of the resource.

We must be careful not to conflate what an URI represents. Does it represent the thing itself, or a document describing the thing? The following example might help grasp this point.
...
First, the real-world concept of a _cat_ needs a URI so that it becomes a resource on the web I can refer to:
...
The `animals` part of the URI reflects the ontology in which `Cat` is defined. This _data resource_ can be found at:
...
[NOTE]
--
Other models and web pages can make statements about cats and use the definition of the ontology above simply by using the `\https://id.example.com/animals/Cat` URI to refer to it.
--

.Takeaway point
TIP: **Things and documents providing descriptions of those things are different (kinds of) resources, each with their own URI**.
...
This becomes a powerful mechanism when retrieving resources using their identity URI, using 303 redirects which -- based on the provided request headers by the <<USER-AGENT,user agent>> -- to negotiate the appropriate representation which are served at different URLs. Typical parameters for negotiating content are language, media type or format, and version.

.Example content negotiation for a real-world entity resource
image::conneg-real-world-entities.svg[]

Note that information resources too can have _several_ representations, even though they are already informational in essence. No different from the previous example, here too, the identity URI is used for retrieval:
...
NOTE: Of course, if one knows the URI of some desired representation, this URI can be used directly instead of using conneg.

== URI syntax
<<def-resource,Resources>> and their representations are identified by URIs, each of which _MUST_ be of the following syntax:
...
.Generic (version-less) data product URIs
[cols="h,3",width=99%]
|===
| Identity
...
NOTE: Do not confuse the category _documentation_ with documentation representations as obtained through using `www` URIs. See also: {fn-doc-iri}

.URI syntax for a project
[source,subs="attributes"]
....
https://www.{domain}/documentation/\{namespace}[/\{version}]
....

.URI syntax for a part
[source,subs="attributes"]
....
...
.Generic (version-less) documentation URIs
[cols="h,3",width=99%]
|===
| Identity
...
.URI syntax for an ontology
[source,subs="attributes"]
....
https://data.{domain}/ontology/\{namespace}[/\{version}]
....

.URI syntax for an ontology term
[source,subs="attributes"]
....
...
IMPORTANT: Never specify versions in the URIs of model elements which represent a real-world concept, not even the model version.

.Example: Data product _Netburen_ schema
[%collapsible]
====

.Generic (version-less) data product schema URIs
[cols="h,3",width=99%]
|===
| Identity
...
.Versioned data product schema URIs
[cols="h,3",width=99%]
|===
| Identity
.2+| `\https://data.{domain}/data-product/netburen/2.1.1`
...
classes:
  MarketEvaluationPoint:
    ...
    is_a: UsagePoint
....
<1> The generic URI of the schema, i.e. not a specific version. Note that we have encoded the fact that this schema is tightly coupled to a data product in the namespace of the URI.
<2> The version of the schema this document represents.
<3> Technical name of the schema. This has local scope, but it is good practice to keep it equal to the model name in the URI.
<4> Prefixes for easy referring to terms from other models. Note the use of the `id` subdomain because ontology and thesauri terms are not real-world concepts. Also: never use versions in URIs which represent real-world concepts.
...
[%collapsible]
====
.Generic (version-less) ontology URIs
[cols="h,3,3",width=98%]
|===
| | Ontology | Term
...
.URI syntax for a register
[source,subs="attributes"]
....
https://data.{domain}/register/\{namespace}
....

.URI syntax for a register entity
[source,subs="attributes"]
....
